...
class ReportController extends Controller
{
    ...
    public function archive(Request $request)
    {
        ...
        $items = \App\Models\Archive::where('company_id', $companyId)
    ...
    public function overdue65(Request $request)
    {
        ...
        // Group by cashbox and sort by cashbox name, then by cashier inside
        ...
        foreach ($collection as $r) {
            if (!empty($r['collateral']) && strpos($r['collateral'], 'пр.') !== false) {
                // Rough parse: sum weights by purity tokens "585 пр." etc
                // Note: We already concatenated strings like: "name, 585 пр., 13.81 гр."
                // We'll regex them out
        ...
        // Per-cashbox subtotals only when "all" cashboxes selected
        $groupTotals = [];
        if ($cashboxId <= 0) {
            foreach ($groups as $cashboxName => $items) {
                $g = collect($items);
                // Gold worth for this cashbox
                $gWorth = 0.0;
                foreach ($items as $r) {
                    if (!empty($r['collateral']) && strpos($r['collateral'], 'пр.') !== false) {
                        if (preg_match_all('/(\d{3})\s*пр\.,\s*([0-9]+(?:\.[0-9]+)?)\s*гр\./u', $r['collateral'], $m, PREG_SET_ORDER)) {
