...
class ExpenseHistoryController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($from || $to) {
            // Normalize to inclusive full-day bounds exactly like transfers
            if(!$from) { $from = $to; }
            if(!$to) { $to = $from; }
    ...
    {
        // Restore cashier balance by writing a reversal ledger entry and soft-deleting the expense
        $expense->delete();

        if ((int)$expense->shift_id > 0) {
